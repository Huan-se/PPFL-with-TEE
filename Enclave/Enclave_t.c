...
static sgx_status_t SGX_CDECL sgx_ecall_secure_aggregation_phase(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_secure_aggregation_phase_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_secure_aggregation_phase_t* ms = SGX_CAST(ms_ecall_secure_aggregation_phase_t*, pms);
	...
	ecall_secure_aggregation_phase(ms->ms_seed, _in_w_new, _in_w_old, _tmp_model_len, _in_ranges, _tmp_ranges_len, _in_output, _tmp_out_len);
	if (_in_output) {
